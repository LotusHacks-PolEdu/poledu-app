...
def store_chunks(chunks_file: str, index_name: str = "default", db_root: str | None = None):
    """Parse a chunkz.txt file, embed the chunks, and store vectors locally in Chroma.

    Args:
        chunks_file: Absolute path to the chunkz.txt file.
        index_name: Namespace for this run (usually process_code).
        db_root: Optional root directory for local database folders.
    """
    chunks = _parse_chunks_file(chunks_file)
    if not chunks:
...
if __name__ == "__main__":
    ...
    default_chunks = r"sudic.pdf.txt"
