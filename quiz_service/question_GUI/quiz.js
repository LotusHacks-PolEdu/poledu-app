...
async function loadByCode(code) {
  ...
  if (!normalized) {
    showLoadError('Please enter an access code.');
    return;
  }

  document.getElementById('landing-subtitle').textContent = 'Downloading test from server...';
  showLoadError('');
  ...
  try {
    ...
    showLoadError(err.message || 'Failed to load test.');
    document.getElementById('landing-subtitle').textContent = 'Load your generated test using access code';
  }
}
...
document.getElementById('file-input').addEventListener('change', (e) => {
  ...
  reader.onload = (ev) => {
    try {
      ...
      showLoadError(`Invalid JSON file: ${err.message}`);
    }
  };
...
function initTest() {
  ...
  document.getElementById('mcq-points').textContent = `${mcqSection.total_points || 0} pts (${mcqSection.points_each || 0} each)`;
  document.getElementById('tf-points').textContent = `${tfSection.total_points || 0} pts (${tfSection.points_each || 0} each)`;
  document.getElementById('short-points').textContent = `${shortSection.total_points || 0} pts (${shortSection.points_each || 0} each)`;
...
function renderMCQ(questions) {
  const section = document.getElementById('section-mcq');
  const container = document.getElementById('mcq-questions');
  container.innerHTML = '';
  section.style.display = questions.length ? '' : 'none';
    ...
    card.innerHTML = `
      ...
      <div class="options-list">
        ${(q.options || []).map((opt) => {
          ...
          return `<button class="option-btn" data-qid="${q.id}" data-letter="${letter}" onclick="selectMCQ(${q.id}, '${letter}')"><span class="option-letter">${letter}</span><span>${opt.substring(3)}</span></button>`;
        }).join('')}
      </div>
    `;
...
function renderShort(questions) {
  const section = document.getElementById('section-short');
  ...
  questions.forEach((q) => {
    ...
    card.innerHTML = `
      ...
      <div class="question-topic">${q.topic || ''}</div>
      <div class="question-text">${q.question}</div>
      <textarea class="short-answer-input" data-qid="${q.id}" placeholder="Type your answer..." oninput="updateShortAnswer(${q.id}, this.value)"></textarea>
    `;
    container.appendChild(card);
  });
}
...
function selectTF(qid, value) {
  ...
  document.querySelectorAll(`#tf-${qid} .tf-btn`).forEach((btn) => {
...
document.getElementById('submit-btn').addEventListener('click', () => {
  if (!confirm('Submit now? MCQ and T/F will be auto-graded.')) return;
  gradeAndShowResults();
});
...
function gradeAndShowResults() {
  ...
  else if (pct >= 70) subtitle = 'Good job!';
  else if (pct >= 50) subtitle = 'Not bad, keep practicing.';
  else subtitle = 'Keep studying, you can do it.';
  ...
  document.getElementById('breakdown-mcq-score').textContent = `${mcqCorrect}/${mcqQuestions.length} (${formatScore(mcqScore)} pts)`;
  document.getElementById('breakdown-tf-score').textContent = `${tfCorrect}/${tfQuestions.length} (${formatScore(tfScore)} pts)`;
  document.getElementById('breakdown-short-score').textContent = 'Self-graded';

  renderReview(mcqResults, tfResults, shortResults);
  showScreen('results-screen');
}

function renderReview(mcqResults, tfResults, shortResults) {
  const container = document.getElementById('review-container');
  container.innerHTML = '';

  if (mcqResults.length > 0) {
    container.innerHTML += '<h3 class="review-subhead">Multiple Choice</h3>';
  ...
  if (tfResults.length > 0) {
    container.innerHTML += '<h3 class="review-subhead">True / False</h3>';
    tfResults.forEach((q) => {
      const statusClass = q.isCorrect ? 'correct' : 'incorrect';
      const statusText = q.isCorrect ? 'Correct' : 'Incorrect';
  ...
  if (shortResults.length > 0) {
    container.innerHTML += '<h3 class="review-subhead">Short Answer</h3>';
    shortResults.forEach((q) => {
      container.innerHTML += `
        <div class="review-card self-grade" id="short-review-${q.id}">
...
function selfGrade(qid, gotRight, points) {
  ...
  if (prompt) {
    prompt.outerHTML = gotRight
      ? `<div class="self-grade-resolved yes">You marked this correct (+${points} pts)</div>`
      : '<div class="self-grade-resolved no">You marked this incorrect (0 pts)</div>';
  }
